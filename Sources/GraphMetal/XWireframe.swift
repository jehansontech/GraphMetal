...
public struct XWireframeUpdate: Codable, Sendable {
    ...
    public static var emptyGraph: XWireframeUpdate {
        XWireframeUpdate(nodeCount: 0, edgeIndexCount: 0)
    }
    ...
    public mutating func merge(_ update: XWireframeUpdate) {
...
public struct XWireframeUniforms {
    var projectionMatrix: float4x4
    var modelViewMatrix: float4x4
    var pointSize: Float
    var edgeColor: SIMD4<Float>
    var fadeoutMidpoint: Float
    var fadeoutDistance: Float
    var pulsePhase: Float
}

public struct XWireframeShaders {
...
public class XWireframe: XRenderable {

    let referenceDate = Date()

    /// The 256 byte aligned size of our uniform structure
    let alignedUniformsSize = (MemoryLayout<XWireframeUniforms>.size + 0xFF) & -0x100

    public var settings: XWireframeSettings
    ...
    var nodeFragmentFunctionName: String {
        ...
        case .disc:
            return "x_node_fragment_disc"
        case .ring:
            return "x_node_fragment_ring"
        case .square:
            return "x_node_fragment_square"
        case .diamond:
            return "x_node_fragment_diamond"
        default:
            return "x_node_fragment_square"
        }
    }
    ...
    var uniforms: UnsafeMutablePointer<XWireframeUniforms>!

    private var isSetup: Bool = false

    private var bufferUpdate: XWireframeUpdate? = nil
    ...
    func setup(_ view: MTKView) throws {
        ...
        if let device = view.device {
            ...
            throw XRenderError.noDevice
        }

        if let device = view.device,
           let library = XWireframeShaders.makeLibrary(device) {
            self.library = library
            // debug("Wireframe.setup", "library functions: \(library.functionNames)")
        }
        else {
            throw XRenderError.noDefaultLibrary
        }
    ...
    public func addBufferUpdate(_ bufferUpdate: XWireframeUpdate?) {
    ...
    public func updateBuffers(_ bufferUpdate: XWireframeUpdate) {
        if self.bufferUpdate == nil {
            // print("Wireframe.updateBuffers: replacing bufferUpdate")
            self.bufferUpdate = bufferUpdate
        }
    ...
    public func prepareToDraw(_ mtkView: MTKView, _ renderSettings: XRenderSettings) {
        ...
        uniformBufferRotation = (uniformBufferRotation + 1) % XRenderConstants.maxBuffersInFlight

        uniformBufferOffset = alignedUniformsSize * uniformBufferRotation

        uniforms = UnsafeMutableRawPointer(dynamicUniformBuffer.contents() + uniformBufferOffset).bindMemory(to:XWireframeUniforms.self, capacity:1)
    ...
    private func buildUniforms() throws {
        let uniformBufferSize = alignedUniformsSize * XRenderConstants.maxBuffersInFlight
        if let buffer = device.makeBuffer(length: uniformBufferSize, options: [MTLResourceOptions.storageModeShared]) {
            ...
            self.uniforms = UnsafeMutableRawPointer(dynamicUniformBuffer.contents()).bindMemory(to:XWireframeUniforms.self, capacity:1)
        }
        else {
            throw XRenderError.bufferCreationFailed
        }
    }

    private func buildNodePipeline(_ view: MTKView) throws {

        let vertexFunction = library.makeFunction(name: "x_node_vertex")
    ...
    private func buildEdgePipeline(_ view: MTKView) throws {
        ...
        let vertexFunction = library.makeFunction(name: "x_edge_vertex")
        let fragmentFunction = library.makeFunction(name: "x_edge_fragment")
...
public struct XWireframeUpdateGenerator {
    ...
    public mutating func makeUpdate<GraphType: Graph>(_ graph: GraphType,
                                                      _ change: XWireframeGraphChange) -> XWireframeUpdate?
    ...
    private mutating func makeNodeSetUpdate<GraphType: Graph>(_ graph: GraphType,
                                                              _ makeEdgeIndices: Bool) -> XWireframeUpdate
    where GraphType.NodeType.ValueType: EmbeddedValue & ColoredValue
    {
        ...
        var update = XWireframeUpdate(bbox: newBBox,
    ...
    where GraphType.NodeType.ValueType: ColoredValue
    {
        ...
        self.nodeIndices = newNodeIndices
        var update = XWireframeUpdate(nodeColors: newNodeColors)
    ...
    private mutating func makeEdgeSetUpdate<GraphType: Graph>(_ graph: GraphType) -> XWireframeUpdate
    {
        ...
        return XWireframeUpdate(edgeIndexCount: newEdgeIndices.count,
...
public struct XWireframeGraphChange: Codable, Sendable {

    public static let none = XWireframeGraphChange()

    public static let all = XWireframeGraphChange(nodes: true,
                                                  nodeColors: true,
                                                  nodePositions: true,
                                                  edges: true,
                                                  edgeColors: true)

    public static let topology = XWireframeGraphChange(nodes: true,
                                                       nodeColors: false,
                                                       nodePositions: false,
                                                       edges: true,
                                                       edgeColors: false)

    public static let geometry = XWireframeGraphChange(nodes: false,
                                                       nodeColors: false,
                                                       nodePositions: true,
                                                       edges: false,
                                                       edgeColors: false)

    public static let colors = XWireframeGraphChange(nodes: false,
                                                     nodeColors: true,
                                                     nodePositions: false,
                                                     edges: false,
                                                     edgeColors: true)

    public static let nodeColors = XWireframeGraphChange(nodes: false,
                                                         nodeColors: true,
                                                         nodePositions: false,
                                                         edges: false,
                                                         edgeColors: false)

    public static let edgeColors = XWireframeGraphChange(nodes: false,
                                                         nodeColors: false,
                                                         nodePositions: false,
                                                         edges: false,
                                                         edgeColors: true)

    public static let nodes = XWireframeGraphChange(nodes: true,
                                                    nodeColors: false,
                                                    nodePositions: false,
                                                    edges: false,
                                                    edgeColors: false)

    public static let edges = XWireframeGraphChange(nodes: false,
                                                    nodeColors: false,
                                                    nodePositions: false,
                                                    edges: true,
                                                    edgeColors: false)
    ...
    public mutating func merge(_ change: XWireframeGraphChange) {
